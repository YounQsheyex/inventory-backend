  ...
  try {
    ...
    const [
      ...
      Sale.find({ status: "completed" })
        ...
        .limit(5)
        .select(
          "invoiceNumber customer total grossProfit saleDate paymentMethod",
        ),
    ]);
...
router.get("/pnl", async (req, res) => {
  try {
    ...
    const [salesData, expenseData, summary, expenseSummary, byCategory] =
      await Promise.all([
        Sale.aggregate([
          {
            $match: {
    ...
    res.json({
      ...
      summary: {
        ...
        totalExpenses,
        netProfit,
        netMargin: s.totalRevenue ? (netProfit / s.totalRevenue) * 100 : 0,
      },
